...
abstract class AbstractNode(open val configuration: NodeConfiguration,
    ...
    private fun <T : FlowLogic<*>> registerInitiatedFlowInternal(initiatedFlow: Class<T>, track: Boolean): Observable<T> {
        ...
        val observable = if (track) {
            smm.changes.filter { it is StateMachineManager.Change.Add }.map { it.logic }.ofType(initiatedFlow)
        } else {
            Observable.empty()
        }

        flowFactories[initiatingFlow] = InitiatedFlowFactory.CorDapp(version, { ctor.newInstance(it) })
    ...
    @VisibleForTesting
    fun registerFlowFactory(initiatingFlowClass: Class<out FlowLogic<*>>, flowFactory: InitiatedFlowFactory) {
